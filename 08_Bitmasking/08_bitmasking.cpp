...
void bitwise() {
	...
	cout << (5 ^ 7) << endl; // Binary XOR => 같으면 0, 다르면 1
	int x = 0;
...
void clearLastIBits(int& n, int i) { // i 개의 비트를 삭제
...
void clearBitsInRange(int& n, int i, int j) { // i 인덱스 ~ j 인덱스 범위의 비트들을 삭제한다
	...
	int b = (1 << i) - 1; // 뒤에 0 을 i 개 만들고 1 이 i 번째에 존재 => 1을 빼니까 => i 번째까지 0, 그 뒤로는 1
...
void replaceBits(int& n, int i, int j, int m) { // n 의 i ~ j 범위 비트를 m 의 비트로 대체하기
	...
	int mask = (m << i);	   // m 을 왼쪽으로 i 만큼 이동 => n 과 m 이 신발 높이가 맞는다
...
지수를 binary bits 로 생각 => O(log n) 으로 빠르게 계산할 수 있다
*/
int fastExpo(int a, int n) {
	...
	while (n > 0) {
		...
		if (last_bit) { // 1 인 경우에
			ans = ans * a; // 현재 밑의 값을 곱해준다
		}
		a = a * a; // 밑을 계속 제곱 => 지수 binary bit 에 맞춰서 진행
		n = n >> 1; // 지수는 다음 자리 수로 계속 넘어가
	}
	return ans;
}

/*
n 의 binary bits 표현을 숫자로 바꿔서 리턴
*/
