...
// 생각나서 만들어본, 중복 조합을 만드는 함수
void test2(vector<string>& vec, array<char, 100>& input, int inputSize, array<char, 100>& output, int outputInd) {
	if (outputInd == inputSize) { // 더이상 조사할 문자가 없다
		output[outputInd] = '\0';
		vec.push_back(string(output.data()));
		return;
	}
	...
	for (int i = 0; i < inputSize; i++) {
		...
		// sub problem 을 만든다, 더 작은 문제로 줄인다
		// 중복 조합의 경우, 들어올 수 있는 input 의 사이즈는 줄어들지 않는다
		// output 에 문자를 하나 넣었으니까, output 에 들어갈 수 있는 크기만 줄어들게 된다
		test2(vec, input, inputSize, output, outputInd + 1);
	}
}
int main() {
	...
	array<char, 100> input{ "ACX" };
	array<char, 100> output{};
	vector<string> str{};

	test2(str, input, 3, output, 0);
	for (auto x : str)
		cout << x << endl;
}